...
const main = async function () {
  ...
  const save_state_async = function (ids, state) {
    queue.save_state(ids, state).catch((err) => {
      console.error(
        "Database error when setting",
        state,
        "on operation with ids:",
        JSON.stringify(ids)
      );
    });
  };
  ...
  const heartbeat = async function () {
    ...
    try {
      ...
      console.log(
        "Sent operation group with hash",
        op_hash,
        "containing operations with ids:",
        JSON.stringify(batched_ids)
      );
      ...
      console.error(
        "An unhandled error has occurred when processing operations with ids:",
        JSON.stringify(batched_ids),
        "\n",
        err,
